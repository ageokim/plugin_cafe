...
| `pm org remove <org>` | 등록 해제 + **그 org의 설치본(clone·링크·활성) 전부 삭제**(§12.2) — 웹 UI는 확인 대화, CLI는 즉시 | |
...
- **org 제거(✕) = 설치본 포함 정리**: 확인 대화에 삭제될 설치본 수를 명시("설치본 N개 포함 삭제할까요?")한 뒤, 그 org의 설치본을 전부 uninstall(비활성→등록 해제→clone 삭제)하고 org를 목록에서 뺀다 — "org를 지운다 = 그 org 것을 더 안 쓴다"는 직관에 맞춤(2026-07-18 재결정, 이전의 '미등록 org 그룹 유지'는 폐기). 크래시 등으로 **비정상 잔존한 폴더**만 '미등록' 그룹·`pm inspect` 플래그로 표면화된다(안전망).
